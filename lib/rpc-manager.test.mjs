...
test("set_thinking_level asserts the level even when it does not change", async () => {
  // pi emits thinking_level_select only on an actual change (agent-session.js:1300 gates it on
  // `isChanging`), so re-selecting the level already in force is invisible to extensions. This
  // test pins the compensating emit: pi-web declares the assertion unconditionally.
  const state = { thinkingLevel: "high" };
  const emitted = [];
  const wrapper = new AgentSessionWrapper({
    sessionId: "s",
    sessionFile: "f",
    ...
    setThinkingLevel(level) {
      state.thinkingLevel = level; // stands in for pi's clamp + in-place write
    },
    extensionRunner: {
      emit: async (event) => {
        emitted.push(event);
  ...
  // (a) a level that does NOT change — the case pi cannot report
  try {
    await wrapper.send({ type: "set_thinking_level", level: "high" });
    assert.equal(emitted.length, 1, "an unchanged level must still assert");
    assert.equal(emitted[0].type, "thinking_level_select");
    assert.equal(emitted[0].asserted, true, "asserted is the sole authorship discriminator");
    assert.equal(emitted[0].level, "high");
    assert.equal(emitted[0].previousLevel, "high");

    // (b) a level that CHANGES — exactly one emit from pi-web, and previousLevel is the OLD value
    await wrapper.send({ type: "set_thinking_level", level: "off" });
    assert.equal(emitted.length, 2);
    assert.equal(emitted[1].level, "off");
    assert.equal(emitted[1].previousLevel, "high", "previousLevel must be sampled BEFORE the write");
    assert.equal(emitted[1].asserted, true);
  } finally {
    wrapper.destroy(); // clears the idle timer, or the test process never exits
  }
});

test("every level button reports its click, including the already-active one", async () => {
  // The component is the FIRST of two suppressions: `if (!isActive)` meant a click on the
  // highlighted level never reached the handler, so no RPC was sent and no assertion could be
  // declared. The hook sends unconditionally once the click lands; this pins that the component
  // no longer swallows it.
  const source = await readFile(new URL("../components/ChatInput.tsx", import.meta.url), "utf8");
  const start = source.indexOf("const isActive = (thinkingLevel");
  assert.ok(start > -1, "the level-button block must still exist (isActive was renamed?)");
  const button = source.slice(start, start + 2000);
  assert.match(button, /onThinkingLevelChange\(lvl\)/);
  assert.doesNotMatch(
    button,
    /if \(!isActive\)\s*onThinkingLevelChange/,
    "the click must not be gated on isActive — re-selecting the active level is the assertion",
  );
});
