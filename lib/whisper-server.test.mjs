...
test("resolveWhisperConfig derives defaults from a sibling Whisper-VTT dir", () => {
  const base = isWin ? "C:\\dev\\pi-web" : "/dev/pi-web";
  const cfg = resolveWhisperConfig({}, base);
  assert.equal(cfg.rootDir, path.join(path.dirname(base), "Whisper-VTT"));
  assert.equal(
    cfg.serverPath,
    path.join(path.dirname(base), "Whisper-VTT", "whisper-cli", "Release", "whisper-server.exe"),
  );
  assert.equal(cfg.modelDir, path.join(path.dirname(base), "Whisper-VTT", "models"));
